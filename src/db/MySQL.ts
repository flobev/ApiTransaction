...
import Utilisateur from "../models/Tuteur";
import Utilisateur from "../models/Enfant";
...
export default abstract class MySQL {
   ...
   * @static
   * @param {string} table
   * @param {(Utilisateur | Tuteur | Enfant)} insert
   * @returns {Promise < number >}
   * @memberof MySQL
   */
  static insert(table: string, instance: Utilisateur): Promise<number> {
    return new Promise((resolve, reject) => {
      // return Promise because the processing time of the database | The only way to get an answer is the "resolve()" or "reject()"
      ...
      const bdd: Connection = createConnection({
        ...
        port: parseInt(
          process.env.PORTMYSQL === undefined ? "3306" : process.env.PORTMYSQL
        ), // 3306 port default to mysql
      });
      bdd.connect((err) => {
      ...
      for (const [key, value] of Object.entries(instance)) {
        // Convert the properties of our objects to an array
        if (instance.attributInsert.indexOf(key) !== -1) {
          // Check to property to the key array because the children Object will acces property parent
          columns += "`" + key + "`,";
          parameters += "?,";
          data.push(value);
        }
      ...
      bdd.query(
        ...
        (error, results, fields) => {
          ...
          if (error) {
            ...
            console.log(error);
          } else resolve(results.insertId); // Reponse promise true => then or await
   ...
   *
   * @static
   * @param {('Utilisateur' | 'Tuteur' | 'Enfant')} table
   * @param {*} [where]
   * @returns {*}
   * @memberof MySQL
   */
  static select(table: listeTables, where?: any): any {
    return new Promise((resolve, reject) => {
      ...
      const query = bdd.query(
        `SELECT ${columns} FROM ${table} WHERE ${conditionWhere} ;`,
        [data],
        (error, results, fields) => {
          // excute request sql
          if (error) {
            reject(error); // Reponse promise false => catch
            console.log(error);
          } else resolve(results); // Reponse promise true => then or await
          bdd.end(); // Close database
        }
      );
    });
  ...
  static selectJoin(
    ...
    return new Promise((resolve, reject) => {
      ...
      const query = bdd.query(
        `SELECT ${columns} FROM ${table} ${conditionJoin} WHERE ${conditionWhere} ;`,
        [data],
        (error, results, fields) => {
          // excute request sql
          if (error) {
