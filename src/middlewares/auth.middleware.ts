...
export const authMidd = (req: Request, res: Response, next: () => void) => {
  ...
  try {
    if (
      ...
      verify(split(req.headers.authorization), <string>process.env.JWT_KEY)
    )
      return next();
    else throw new Error(`Authorization not found`);
  } catch (err) {
  ...
  try {
    ...
    for (const require in champsRequire) {
      ...
      for (const champs in data) {
        if (champs === champsRequire[require]) error = false;
      }
      if (error) textError += `${champsRequire[require]}, `;
    }
